# 1 "c:\\Users\\peera\\Desktop\\robotDemo\\robotArm\\robotArm.ino"
# 1 "c:\\Users\\peera\\Desktop\\robotDemo\\robotArm\\robotArm.ino"
/*
...
*/
# 62 "c:\\Users\\peera\\Desktop\\robotDemo\\robotArm\\robotArm.ino"
...
# 83 "c:\\Users\\peera\\Desktop\\robotDemo\\robotArm\\robotArm.ino" 2
...
void setup()
{
  ...
  ledcSetup(3, 50, 16); // channel 3, 50 Hz, 16-bit width
  ledcAttachPin(23, 3); // GPIO 23 assigned to channel 3

  ledcSetup(4, 50, 16); // channel 4, 50 Hz, 16-bit width
  ledcAttachPin(18, 4); // GPIO 18 assigned to channel 4

  ledcWrite(1, middle_currPOS);
  ledcWrite(2, left_currPOS);
  ledcWrite(3, right_currPOS);
  ledcWrite(4, claw_currPOS);
}
...
void gotoPOS(int servo, int position)
{
  ...
  {
  case 1:
    if (position > middle_currPOS && position < 6200)
    {
    ...
    else if (position < middle_currPOS && position > 1700)
    {
      while (middle_currPOS > position)
      {
        middle_currPOS -= servo_scale;
  ...
  case 2:
    if (position > left_currPOS && position < 6800)
    {
    ...
    else if (position < left_currPOS && position > 3350)
    {
      while (left_currPOS > position)
      {
        left_currPOS -= servo_scale;
  ...
  case 3:
    if (position > right_currPOS && position < 6200)
    {
    ...
    else if (position < right_currPOS && position > 1900)
    {
      while (right_currPOS > position)
      {
        right_currPOS -= servo_scale;
  ...
  case 4:
    if (position > claw_currPOS && position < 6000)
    {
    ...
    else if (position < claw_currPOS && position > 2300)
    {
      while (claw_currPOS > position)
      {
        claw_currPOS -= servo_scale;
